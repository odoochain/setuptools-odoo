        ...
        try:
            ...
            with tarfile.open(sdist_file, "r") as tf:
                ...
                try:
                    ...
                    subprocess.check_call(
                        [sys.executable, "setup.py", "sdist"], cwd=tar_setup_dir
                    )
            ...
            try:
                ...
                try:
                    ...
                    with ZipFile(wheel_fname) as zf:
                        ...
                        assert "odoo_addons/addon5/__openerp__.py" in namelist
                        # some non python file was included because #
                        # it is under git control
                        assert "odoo_addons/addon5/data/somedata.xml" in namelist
                        # this file is not under git control,
                        # so its not in the wheel
                        assert "odoo_addons/addon5/data/notingit.xml" not in namelist
                finally:
                    shutil.rmtree(dist_dir)
                    shutil.rmtree(os.path.join(addon5_dir, "build"))
                    shutil.rmtree(
                        os.path.join(addon5_dir, "odoo8_addon_addon5.egg-info")
                    )
            finally:
